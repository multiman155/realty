...
public record AgentRemoveCommand(@NotNull ExecutorState executorState,
    ...
    private void execute(@NotNull CommandContext<CommandSourceStack> ctx) {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                int rows = logic.removeSanctionedAuctioneer(regionId, worldId, targetId);
                if (rows > 0) {
                    sender.sendMessage(messages.messageFor("agent-remove.success",
                            Placeholder.unparsed("player", targetName),
                            Placeholder.unparsed("region", regionId)));
                } else {
                    sender.sendMessage(messages.messageFor("agent-remove.not-found",
                            Placeholder.unparsed("player", targetName),
                            Placeholder.unparsed("region", regionId)));
                }
            } catch (Exception ex) {
                sender.sendMessage(messages.messageFor("agent-remove.error",
